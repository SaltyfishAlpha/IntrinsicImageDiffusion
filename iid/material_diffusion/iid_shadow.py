...
class IntrinsicImageDiffusion(LightningModule):
    ...
    def sample(self,
        ...
        conditioning = Batch()
        conditioning.origin = self._nan_to_num(conditioning_img * 2 - 1).float()
        ...
        if conditioning.origin.shape[0] != batch_size:
            conditioning.origin = conditioning.origin.repeat(batch_size, *([1] * (conditioning.origin.ndim - 1)))
        ...
        #     return shadow_free, x_intermediates
        return shadow_free
