...
from iid.material_diffusion.iid_shadow import IntrinsicImageDiffusion
...
def material_diffusion(cfg: DictConfig):
    ...
    if "input_path" in cfg.data:
        ...
        # Prepare batch
        transforms = Compose([ToTensor(), Resize(size=[512, 512])])
        ...
        for batch in dataloader:
            assert len(batch["origin"]) == 1, "Only a single sample can be loaded now"
            img = batch["origin"].to(device)
            original_size = batch[0]["metadata"]["size"]["origin"].tolist()[:2]
    ...
    if "output" in cfg:
        ...
        if not cfg.output.as_dataset:
            # Save with suffix
            to_pil(preds).save(os.path.join(out_folder, f"{image_id}_albedo.png"))
        else:
            # Save as dataset
...
@hydra.main(version_base="1.3", config_path="../../configs", config_name="test_shadow.yaml")
